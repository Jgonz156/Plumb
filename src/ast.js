...
const astBuilder = plumbGrammar.createSemantics().addOperation("ast", {
    ...
    TypeParameterPairDec(prototype, id) {
        return new core.TypeParameterPairDec(prototype.sourceString, id.ast())
    },
    PrototypeDec(_proto, id, block) {
        return new core.PrototypeDec(id.ast(), block.ast())
    },
    ...
    IfStatement(_if, _lp, expression, _rp, block) {
        return new core.IfStatement(expression.ast(), block.ast())
    },
    WhileStatement(_while, _lp, expression, _rp, block) {
        return new core.WhileStatement(expression.ast(), block.ast())
    },
    ForStatement(
        _for,
        _lp,
    ...
    KeyValuePair(key, _colon, value) {
        return new core.KeyValuePair(key.ast(), value.ast())
    },
    ...
    Exp7_Index(left, _lb, exp, _rb) {
        return new core.IndexExpression(left.ast(), exp.ast())
    },
    Exp7_Call(left, _lp, args, _rp) {
        return new core.Call(left.ast(), args.asIteration().ast())
    },
    ...
    Exp7_Access(left, _dot, id) {
        return new core.AccessExpression(left.ast(), id.ast())
    },
    ...
    Pipelines(_pipeline, _lb, pipes, _rb) {
        return new core.Pipelines(pipes.ast())
    },
